    ...
    $( "#dialog-confirm" ).dialog({
      ...
      buttons: {
        "Delete node" : function() {
          var indexToDelete = ciachoMap.data.findIndex(function(x){return x.id === selected.obj.id;});
          
          if (typeof indexToDelete !== 'undefined') {
            ciachoMap.data.splice(indexToDelete, 1);
            
            for(var i = 0; i < ciachoMap.data.length ; i+=1) {
              var dependsToRemove = ciachoMap.data[i].depends.findIndex(function(x){return x === selected.obj.id;});
...
function visualizeTypeCenter() {
  ...
  var conterDrag = d3.behavior.drag()
        ...
        .on('drag', function(d) {
          ...
          var typeObj = config.constraints.find(function(x){return x.has.type === typeName;})
